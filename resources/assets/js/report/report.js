        ...
        success: function (result) {
            ...
            for (const key in projects) {
                if (projects.hasOwnProperty(key)) {
                    $('#projectIds').
                        append(
                            $('<option>', { value: key, text: projects[key] }))
                }
            }
        },
    })
...
let tbl = $('#report_table').DataTable({
    ...
    columns: [
        ...
        {
            data: function (row) {
                return '<a title="Run Report" class="btn action-btn btn-success btn-sm mr-1" href="' +
                    reportUrl + row.id + '">' +
                    '<i class="fas fa-eye action-icon"></i>' + '</a>' +
                    '<a title="Edit" class="btn action-btn btn-primary btn-sm edit-btn mr-1" href="' +
                    reportUrl + row.id + '/edit">' +
